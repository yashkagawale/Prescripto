  ...
  return (
    <form onSubmit={OnSubmitHander} className='min-h-[80vh] flex items-center'>
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border-0 rounded-xl text-zinc-600 text-sm shadow-lg">
        ...
        {
          state === "Sign Up"
          ? (<p>Already have an account? <span onClick={() => setState('Login')} className='text-indigo-500 cursor-pointer'>Login here</span></p>)
          : (<p>Create a new account? <span onClick={() => setState('Sign Up')} className='text-indigo-500 cursor-pointer'>Click here</span></p>)
        }
      </div>
    </form>
  )
